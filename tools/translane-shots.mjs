/** Roads epic R2 validation: build a bus-lane column and a bike-lane column,
 * each crossed by a two-lane street (to prove the colored band breaks at
 * junctions), then shoot street-level close-ups so the terracotta bus lane +
 * transit diamond and the green bike lane + bicycle glyph are visible. */
import { chromium } from 'playwright';
import { mkdirSync } from 'node:fs';
const base = process.argv[2] ?? 'http://localhost:5173';
const url = base + (base.includes('?') ? '&' : '?') + 'nobloom';
const out = process.argv[3] ?? 'tools/shots-translane';
mkdirSync(out, { recursive: true });
const RT = { TwoLane: 1, BusLane: 8, BikeLane: 9 };
...
let A = null;
for (let z = 25; z < N - 30 && !A; z++) for (let x = 25; x < N - 30 && !A; x++) { const h0 = g.height[idx(x, z)]; let ok = true; for (let dz = -2; dz < 18 && ok; dz++) for (let dx = -2; dx < 18 && ok; dx++) if (!okTile(x + dx, z + dz, h0)) ok = false; if (ok) A = { x, z }; }
...
// Bus-lane column + bike-lane column, both crossed by a two-lane street.
await vcol(X + 3, Z, Z + 14, RT.BusLane);
await vcol(X + 11, Z, Z + 14, RT.BikeLane);
await hrow(Z + 7, X, X + 14, RT.TwoLane);
...
await shot('bus-straight', X + 3, Z + 2, 15);
await shot('bus-junction', X + 3, Z + 7, 18);
await shot('bike-straight', X + 11, Z + 2, 13);
await shot('bike-junction', X + 11, Z + 7, 16);
await shot('overview', X + 7, Z + 7, 60);
console.log('done');
await b.close();
