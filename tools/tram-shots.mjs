/** Roads epic R3 validation: build a tram-track column crossed by a two-lane
 * street, then shoot street-level close-ups so the embedded steel rails +
 * cross-tie sleepers are visible and the track breaks cleanly at the junction. */
...
let A = null;
for (let z = 25; z < N - 30 && !A; z++) for (let x = 25; x < N - 30 && !A; x++) { const h0 = g.height[idx(x, z)]; let ok = true; for (let dz = -2; dz < 18 && ok; dz++) for (let dx = -6; dx < 10 && ok; dx++) if (!okTile(x + dx, z + dz, h0)) ok = false; if (ok) A = { x, z }; }
...
await vcol(X, Z, Z + 14, RT.Tram);
await hrow(Z + 7, X - 5, X + 5, RT.TwoLane);
...
await shot('tram-straight', X, Z + 2, 13);
await shot('tram-junction', X, Z + 7, 18);
await shot('tram-overview', X, Z + 7, 44);
console.log('done');
await b.close();
