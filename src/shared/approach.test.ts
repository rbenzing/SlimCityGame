...
describe('the turn pocket an approach earns', () => {
  ...
  it('is not warranted for an arm the control does not hold', () => {
    // A minor-road stop queues the side street and lets the road through. The
    ...
    // A stop holds only the arms below the top rank.
    expect(controlHoldsArm('stop', 2, [2, 20])).toBe(true);
    expect(controlHoldsArm('stop', 20, [2, 20])).toBe(false);
    // Where every arm ranks the same there is no road running through, so a
