...
game-units constant — see [../SPEC.md](../SPEC.md) for the per-class numbers.
...
`shared/junction.ts`; see [../SPEC.md](../SPEC.md) for their shape). That base
...
unlike garbage or service dispatch, traffic has no cadence divisor.
...
- `tripsForTick(population, tickNo)` sets the trip budget for this tick: a
  ...
  `NIGHT_ACTIVITY_FLOOR` overnight (see [../SPEC.md](../SPEC.md) for the
...
see [../SPEC.md](../SPEC.md) for those effects. The vehicle buffer
