...
| `buildSegment`         | `tier`, `profile?`, `a`, `b`, `control?`, `flow?` (points in world cm)         | `costPerTile` × length ÷ 20 m                           | `removeSegment` of the same ends and control                                          |
| `removeSegment`        | `a`, `b`, `control?`                                                           | negative (50% of `costPerTile` × length ÷ 20 m)         | `buildSegment` of what was removed                                                    |
...
internal shape `src/world/roadgraph.ts`'s `RoadNetwork` exposes to other worker
...
| `readApproach`        | `(x, z) => {...} \| null`                                | the junction a tile approaches and its cross-section there (lanes, width, pocket, taper, auxiliary lane), plus `allowed` — the movement set every lane-use arrow on the arm is painted from, which settles which way an arrow points without reading a metre of paint off an oblique screenshot                                                                                                     |
| `readDrawn`           | `(x, z) => {...} \| null`                                | the same question put to the **mesh** instead of the grid — the two can disagree                                                                                                                                                                                                                                                                                                                    |
| `readPaint`           | `(x, z) => { color, axis, from, to, tris, near, far }[]` | the continuous bands a tile actually lays, measured off the chunk's vertex buffer — how wide the asphalt and paint came out, not how wide the section says they are. `near`/`far` give the band's position at each END of the tile: equal means it runs straight, differing means it is easing across, and comparing one tile's `far` with the next tile's `near` is what tells a taper from a step |
| `readSurface`         | `(x0, z0, x1, z1, n) => (string \| null)[]`              | the same geometry asked point by point instead of band by band: what covers each sample of a rectangle of ground, topmost surface first, `null` where the terrain shows through. Row-major, `n` per side. A kerb return is a couple of metres of corner and reaches no tile edge, so it is invisible to `readPaint` and had been checkable only by eye                                              |
| `readSignals`         | `() => { mast, head, axis, side, tile }[]`               | where each signal's mast stands and where its head hangs, in world metres, through the same transform the renderer writes into the instance matrix. A mast arm is foreshortened to nothing in an overhead shot, so whether a head reaches out over the lanes it holds is measured here rather than guessed at from a picture                                                                        |
