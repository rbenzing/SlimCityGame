...
`recovery: { id, units }[]` and an optional `recoveredTotal`, and `restoreState`
already ignores an absent state. The rule holds unchanged: **a save written
...
vehicle. No new `Command`; `paintLandfill` is untouched. See
...
it means now; the forwarding facilities set it false.
...
readings, so the panel reports load as it does for a clinic without a second
source of truth. Waste gets **no** `ServiceKind` member and no coverage field:
the umbrella already rejected a field per service on memory grounds, and
...
and both live in `src/shared/constants.ts`:
...
One rule, applied in `GarbageSystem.tick` ahead of the existing disposal passes,
because nothing can be recovered once it is buried:
...
2. Each keeps `floor(collected × diversion)` as **recovered**, carrying the
   remainder per facility rather than per building — 7% of a 2-unit building is
   zero, and a per-building round would make small buildings divert nothing.
...
fleet stays at 4: a render budget, not a capacity, and `MAX_GARBAGE_TRUCKS` is
16 for the whole city.
...
| Formula GFA        | 6×185×(8÷3.2)    | 12×185×(9÷3.2)   | 24×185×(11÷3.2)   |
| = volume proxy     | 2,775            | 6,244            | 15,263            |
| Occupant check     | 3 × 9.3 = 28     | 4 × 9.3 = 37     | 15 × 9.3 = 140    |

The drivers. **Recycling centre:** six 30 yd³ roll-offs at 2.4 m wide, each
needing 6.7 m of container and 12 m of hook-lift pull clearance, plus a 7.3 m
aisle and eight 2.7 × 5.5 m stalls; 2×3 rather than 5 tiles, because a 1×5 strip
cannot hold a 26 m service depth, and 8 m is the published hook-lift tipping
height plus door clearance. **Transfer station:** 44 t/day at one unloading
stall per 25–30 t/day is two stalls at 4.0 × 24 m, plus a one-day surge pile at
300 kg/m³ stacked 2 m and one below-grade trailer position at 22 × 4.9 m; 9 m is
inside the published 7.5–9 m tipping-hall clear height and above a rear loader's
5.5 m raised tailgate. **Recovery facility:** 88 t/day of which 21.1% is
...
covers 8 m clear over the tipping floor, a sort platform at 4.5 m and a 9 m
baler bay. In all three the occupant check lands at 1–4% of the plan area — the
...
20 m is 18,500 m², a 300–600 t/day mass-burn plant. `burnRate: 4000` a pass is
80,000 units a day — **20 t/day**, 15 to 30 times too little. `bufferCapacity:
400000` is 100 t, exactly 5 days of that burn against published refuse-pit
storage of 3–5 days, so the buffer is the one figure that is right. The building
is authoritative; `burnRate` is re-derived from it.
...
two days if collection lapses, silently losing what the sim needs. The fix is
structural and belongs here: the store of record becomes a per-building figure
and the per-tile layer becomes a **projection** of it into 0..255 for the lens.
Largest piece of work in the epic, and the least visible.
...
2-unit building is zero, so remainders must carry per facility in integers; and
recovery accumulates every 10 ticks while income settles every 6,000, so the
accumulator must be cleared once at the month boundary or it double-counts.

**A recovery facility with no disposal behind it is a trap** — intended, and it
will read as a bug, so the panel must say _forwarded: 0, no disposal in reach_
rather than leave the player to infer it from a rising buffer.

## Alternatives

**A recycling percentage slider**, or **a split-body collection vehicle.** Both
rejected: replacing one dial with another is what this epic exists not to do,
and a split body would need the cosmetic fleet to model a payload it never had.
...
different quantities, and deriving both from one throughput figure gives the
single source of truth without pretending they are the same number.

**Diversion at source**, and **leaving `LANDFILL_CAPACITY_PER_TILE` at 600 while
scaling generation down.** Both rejected: diverting at source lets a facility
work with nothing behind it, removing the ladder's bottom rung; and low-density
generation is the one number already matching a published figure, the landfill
none.

## How we will know it works

- A save written before this epic loads, with no recovery facilities present and
  the city behaving exactly as it did. **Written first** — it protects every
  existing city.
- A save written before the rescale loads with its landfill fill _fraction_
  unchanged: the stored pile is multiplied by the ratio, not reset.
- A city with a recycling centre in reach buries 7% less than the same city
  without one, over the same number of passes.
- A building reached by both a centre and a recovery facility is diverted once,
  at 21% and not at 28%.
- A recovery facility with no landfill or incinerator reachable collects until
  its buffer is full, then stops, and trash backs up on the source tiles.
  Removing it drops that buffer, as `drop(id)` already does.
- A transfer station forwards to a final facility outside its own
  `collectionRange`, what arrives does not fall with distance, and nothing it
  collects is diverted.
...
- Determinism, and budget: the same city ticked twice from the same state
  recovers the same units in the same facilities, and a profile at the
  performance-budget city size keeps the garbage pass within budget.

**This epic renders, so read-backs are not enough.** In a browser, at the
default camera pitch:

1. **The three facilities in a row**, each with a 4.0 m car and a 9 m refuse
   vehicle on its apron: every bay door taller than the vehicle that uses it, and
   the anchors in [../../art/README.md](../../art/README.md) holding.
2. **The recycling centre at street level**: six roll-off containers and a
   hook-lift shed reading as a drop-off yard, not a small warehouse.
3. **The recovery facility beside the incinerator**: a 4×6 / 11 m hall reading
   lower and longer than a 4×4 / 20 m burner, the silhouette being all that
   tells them apart at distance.
...
- **Composting and organics**, **per-material tracking** and **waste export**:
  the published 8.5 pp is in the design document so the next epic inherits it;
  otherwise one stream, one rate, and no outside world to export to.
- **A payload model for the cosmetic fleet**, and **reworking collection**: the
  BFS, the building-id order and the full-buffer stop rule are unchanged.
- **Retuning the incinerator's mass or truck count** — `burnRate` is corrected
  because diversion depends on it; the building and its four trucks stay.
