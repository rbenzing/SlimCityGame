...
**Buildings** — standard storey 3.2 m. A building's PLAN IS A SIZE, NOT A SHARE OF THE TILE: the body covers a fixed number of metres per tile of its lot — 4.75 m for a detached home, 13.6 m for everything else — and the fill fraction is only that figure divided by whatever the tile currently measures, capped so neighbours never touch. The home figure is per TILE and the smallest ResLow lot is two of them, so a detached frontage is 9.5 m and the largest is 14.25 m; judged against the 4.0 m car at the kerb, anything wider stops reading as a house. Heights are real metres and have never scaled, so holding the plan in metres too is what keeps a building's proportions its own. Held as a fraction instead (which it was, at 0.55 and 0.85), every building in the game changes shape the moment the tile is resized: when the tile went from 16 m to 20 m every plan stretched 25% under an unchanged height and the whole catalogue flattened by that ratio. Whatever the tile does not spend is yard and verge, which is the intended suburban look and not wasted space.

| Zone                     | Storeys      | Eaves height | Roof              | Footprint fill           | Notes                         |
| ------------------------ | ------------ | ------------ | ----------------- | ------------------------ | ----------------------------- |
| ResLow (detached)        | 1–2          | 3.2–6.5 m    | pitched 2.0–3.5 m | 4.75 m per lot tile      | yard + driveway + garage      |
